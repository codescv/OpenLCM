...
def _hermes_compression_threshold(default: float) -> float:
    """Read config.yaml threshold when no LCM_CONTEXT_THRESHOLD env override is present.

    Checks OPENLCM_HOME (or HERMES_HOME for Hermes compat) for a config.yaml.
    """
    home = Path(
        os.environ.get("OPENLCM_HOME")
        or os.environ.get("HERMES_HOME")
        or Path.home() / ".openlcm"
    )
...
class LCMConfig:
    ...
    # -- Storage ---
    database_path: str = ""       # empty = auto (~/.openlcm/lcm.db)
