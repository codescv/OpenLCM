...
def _call_extraction_llm(
    ...
    """Call the configured summary backend for extraction."""
    if backend is None:
        logger.debug("No backend provided for extraction — skipping")
        return None
    try:
        import asyncio
        result = asyncio.run(backend.summarize(prompt, max_tokens=2000, model=model, timeout=timeout))
        if not result:
            return None
...
def extract_before_compaction(
    ...
    try:
        ...
        result = _call_extraction_llm(prompt, backend=backend, model=model, timeout=timeout)
