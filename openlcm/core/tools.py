...
def _state_db_path_for_engine(engine: "LCMEngine") -> Path:
    db_path = Path(getattr(engine._store, "db_path", Path.home() / ".openlcm" / "lcm.db"))
...
def _get_externalized_payload(
    ...
    *,
    ...
    payload = load_externalized_payload(ref, config=engine._config, hermes_home=getattr(engine, "_hermes_home", ""))
    if payload is None:
        return None
...
def _synthesize_expansion_answer(
    *,
    ...
    if backend is None:
        return "lcm_expand_query requires a SummaryBackend. Pass the engine's backend."
    ...
    try:
        result = asyncio.run(backend.summarize(full_prompt, max_tokens=max_tokens, model=model, timeout=timeout))
        if not result:
            return "No answer generated."
        return _strip_reasoning_blocks(result).strip()
    except Exception as exc:
        return f"Expansion synthesis failed: {exc}"
